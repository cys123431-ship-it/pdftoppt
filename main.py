...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.queue_remove_btn = ttk.Button(queue_button_frame, command=self.remove_queue_selection)
    ...
    def _update_queue_hint(self) -> None:
        ...
        self.queue_hint_label.config(text=self._t(hint_key))
    ...
    def _resolved_input_for_operation(self, operation: str | None = None) -> str | tuple[str, ...]:
        ...
        if target_operation == OP_BATCH:
            return self.selected_input if os.path.isdir(self.selected_input) else ""
    ...
    def _refresh_dynamic_controls(self) -> None:
        ...
        self.page_range_entry.config(state=tk.DISABLED if operation == OP_MERGE else tk.NORMAL)
        self.batch_combo.config(state="readonly" if operation == OP_BATCH else tk.DISABLED)
        self.output_password_entry.config(
            state=tk.NORMAL if operation in (OP_MERGE, OP_SPLIT) else tk.DISABLED
        )
        needs_dpi = operation in (OP_PDF_TO_PPTX, OP_PDF_TO_PNG, OP_PDF_TO_JPG, OP_BATCH)
        self.render_dpi_spin.config(state=tk.NORMAL if needs_dpi else tk.DISABLED)
    ...
    def _parse_numeric_options(self, operation: str) -> tuple[int, int] | None:
        render_dpi = 144
        jpg_quality = 90
    ...
    def conversion_finished(self, success: bool, message: str) -> None:
        ...
        localized_message = localize_message(message, self.language_code.get())
        if success:
